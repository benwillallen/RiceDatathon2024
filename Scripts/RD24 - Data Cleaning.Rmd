...
```{r}
lin_model <- glm(formula=OilPeakRate~., data=imputed_data[train_test_split,])
lin_preds <- predict(lin_model, newdata=imputed_data[-train_test_split,])
sqrt(mean((lin_preds-imputed_data$OilPeakRate[-train_test_split])^2))
...
colnames(refined_data) <- c(colnames(refined_data)[1:length(refined_data)-1], "OilPeakRate")
```
